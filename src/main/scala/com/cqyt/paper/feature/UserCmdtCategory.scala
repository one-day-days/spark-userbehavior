...
import org.apache.spark.sql.functions.{avg, col, count, sum, when}
...
object UserCmdtCategory {
  ...
  def build() {
    ...
    val categoryItem = spark.read.orc("file:///home/data/commdity")
    val userItemBehavior = spark.read.orc("file:///opt/feature/userItem/userItemBehavior_cnt")
    ...
    userItemCategoryRatio.write.mode(SaveMode.Overwrite).orc("file:///opt/feature/userItemCategoryRatio")
